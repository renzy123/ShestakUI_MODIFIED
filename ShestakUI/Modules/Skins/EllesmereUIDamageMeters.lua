...
-- 状态更新：持续确保原生底色与原生边框隐藏，维持标题与文本样式及 2px 间距
local function UpdateWindowState(W)
	...
	if W.viewport and header then
		...
		for pIdx = 1, numPoints do
			...
			if point == "TOPLEFT" and relTo == header and relPoint == "BOTTOMLEFT" and (y or 0) > -2 then
				W.viewport:SetPoint("TOPLEFT", header, "BOTTOMLEFT", x or 0, -2)
				break
			end
		end
	end
	if W.sourceFrame and header then
		...
		for pIdx = 1, numPoints do
			...
			if point == "TOPLEFT" and relTo == header and relPoint == "BOTTOMLEFT" and (y or 0) > -2 then
				W.sourceFrame:SetPoint("TOPLEFT", header, "BOTTOMLEFT", x or 0, -2)
				break
			end
		end
	end
...
local function SkinWindow(W)
	...
	if not W._shestakSkinned then
		...
		-- 6. 调整视口与首行统计间距（标题行和第一行统计之间保持 2px 间距）
		local HEADER_GAP = 2

		-- 主视口（主伤害统计行池）：TOPLEFT 锚定在 header 的 BOTTOMLEFT 下方 2px
		...
		-- 置顶玩家行（若开启置顶）：TOPLEFT 同样向下偏移 2px
		...
		-- 来源技能明细视口容器（点击玩家展开的下钻技能列表）：同样向下偏移 2px
		...
		-- 打印适配日志，方便排错与确认
		print("|cff00ff00ShestakUI:|r 成功美化伤害统计窗口 [" .. (W.idx or "?") .. "]，标题栏无左右边框，已应用 2px 标题与首行统计间距。")
	end
